...
// --- TYPES & INTERFACES FOR BACKEND HAND-OFF ---
interface HousekeepingTicket {
  id: string;
  location: string;
...
// --- MODAL COMPONENT (image_4ed542.png) ---
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      ...
      <div className="bg-slate-50 w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden border border-slate-200 flex flex-col max-h-[90vh]">
        ...
        {/* Form Body Context */}
        <form className="p-6 overflow-y-auto space-y-5 text-slate-800" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="bg-white border border-slate-200/60 rounded-xl p-5 space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <select className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-medium focus:outline-none focus:border-[#2B92E4] text-slate-400">
          ...
          <div className="bg-white border border-slate-200/60 rounded-xl p-5 space-y-4 shadow-sm">
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <textarea 
                ...
                className="w-full bg-white border border-slate-200 rounded-lg p-3 text-xs font-medium focus:outline-none focus:border-[#2B92E4] placeholder-slate-300 resize-none min-h-[80px]"
              />
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200/60 rounded-xl p-5 space-y-4 shadow-sm">
            ...
            <div className="max-w-xs space-y-1.5">
              ...
              <select className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-medium focus:outline-none focus:border-[#2B92E4] text-slate-400">
                ...
                <option>Unassigned Staff</option>
              </select>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200/60 rounded-xl p-5 space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input 
                  type="date" 
                  className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-medium focus:outline-none focus:border-[#2B92E4] text-slate-400"
                />
              </div>
              <div className="space-y-1.5">
...
// --- MAIN PAGE MANAGEMENT COMPONENT ---
export default function BuildingAdminHousekeeping() {
  const [tickets] = useState<HousekeepingTicket[]>([
    { id: "TKT - 0024", location: "2nd Floor, Room 201", priority: "Medium", issueType: "Room Cleaning", targetDate: "April 22, 2026", assignedStaff: { name: "Unassigned Staff", isUnassigned: true }, status: "Pending", reportedDate: "April 23, 2026", details: "Common symptoms include slow drains, water discoloration, and low pressure, which can indicate leaks or corrosion within the system.", locationType: "Room - Single Cabin" },
    { id: "TKT - 0023", location: "3rd Floor, Room 301", priority: "High", issueType: "Deep Cleaning", targetDate: "April 23, 2026", assignedStaff: { name: "John Doe", isUnassigned: false }, status: "In Progress", reportedDate: "April 22, 2026" },
    { id: "TKT - 0022", location: "4th Floor, Room 403", priority: "Medium", issueType: "Disinfection", targetDate: "April 23, 2026", assignedStaff: { name: "John Doe", isUnassigned: false }, status: "For Review", reportedDate: "April 22, 2026" },
    { id: "TKT - 0021", location: "2nd Floor, Room 205", priority: "High", issueType: "Bed Reset", targetDate: "April 23, 2026", assignedStaff: { name: "John Doe", isUnassigned: false }, status: "In Progress", reportedDate: "April 22, 2026" },
    { id: "TKT - 0019", location: "4th Floor, Room 403", priority: "Low", issueType: "Linen Change", targetDate: "April 22, 2026", assignedStaff: { name: "John Doe", isUnassigned: false }, status: "Completed", reportedDate: "April 20, 2026", details: "Change linen to maintain high hygiene standards, ensure guest comfort, and remove dirt, bacteria, and allergens that accumulate from daily use.", locationType: "Room - Shared Cabin" },
    { id: "TKT - 0020", location: "3rd Floor, Room 303", priority: "Low", issueType: "Linen Change", targetDate: "April 23, 2026", assignedStaff: { name: "John Doe", isUnassigned: false }, status: "Pending", reportedDate: "April 22, 2026" },
  ]);

  const [searchQuery, setSearchQuery] = useState("");
  const [selectedTicket, setSelectedTicket] = useState<HousekeepingTicket | null>(null);
  ...
  // --- DETAILED TICKET SUB-VIEW PANEL ---
  if (selectedTicket) {
    ...
    // Dynamic numeric logic for context counters
    let progressTabSuffix = "";
    if (isCompleted || isForReview) progressTabSuffix = " 3";
    if (isInProgress) progressTabSuffix = " 1";

    return (
      <div className="flex-1 bg-[#f8fafc] p-8 min-h-screen flex flex-col">
        {/* Breadcrumb Row */}
        ...
        {/* Disabled Warning Banner for Completed View (image_4ee085.png) */}
        {isCompleted && (
        ...
        {/* 2. PROGRESS UPDATE TAB CONTEXT */}
        {activeTab === "Progress Update" && (
          <div className="space-y-6">
            {isPending && (
              <div className="bg-white border border-slate-200/70 rounded-2xl p-16 shadow-sm flex items-center justify-center min-h-[220px] max-w-2xl">
            ...
            {isInProgress && (
              <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm max-w-4xl space-y-4">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center text-[10px] px-2.5 py-0.5 rounded-full font-bold bg-amber-50 text-amber-600 border border-amber-100 uppercase">
                    In Progress
                  </span>
                </div>
        ...
        {/* 3. HISTORY TAB LOG CONTEXT (image_4ee485, image_4ee179, image_4ee085) */}
        {activeTab === "History" && (
          <div className="bg-white border border-slate-200/60 rounded-2xl p-6 shadow-sm max-w-4xl space-y-6">
            ...
            <div className="border-t border-slate-100/80 pt-4 space-y-5">
              {/* IMAGE 1: Pending View State Log Layout */}
              {isPending && (
                <div>
              ...
              {/* IMAGE 2: In Progress View State Log Layout */}
              {isInProgress && (
                <>
                  ...
                  <div className="border-b border-slate-100 pb-4">
                    <span className="block text-xs font-medium text-slate-400">Assigned to <strong className="font-bold text-slate-800">John Doe</strong></span>
                    <span className="block text-[11px] font-medium text-slate-400 mt-0.5">by Building Admin ; April 21, 2026 12:00 PM</span>
                  </div>
                  <div>
                    ...
                    <span className="block text-[11px] font-medium text-slate-400 mt-0.5">by John Doe ; April 21, 2026 12:00 PM</span>
                  </div>
                </>
              )}

              {/* IMAGE 3: Completed View State Log Layout */}
              {(isCompleted || isForReview) && (
                <>
                  <div className="border-b border-slate-100 pb-4">
                    <span className="block text-xs font-bold text-slate-800">Ticket Created</span>
                  ...
                  <div className="border-b border-slate-100 pb-4">
                    <span className="block text-xs font-medium text-slate-400">Task status updated to <strong className="font-bold text-slate-800">In Progress</strong></span>
                    <span className="block text-[11px] font-medium text-slate-400 mt-0.5">by John Doe ; April 21, 2026 12:00 PM</span>
                  </div>
                  <div className="border-b border-slate-100 pb-4">
                    <span className="block text-xs font-medium text-slate-400">Task status updated to <strong className="font-bold text-slate-800">For Review</strong></span>
                    <span className="block text-[11px] font-medium text-slate-400 mt-0.5">by John Doe ; April 21, 2026 12:00 PM</span>
                  </div>
                  <div>
  ...
  // --- STANDARD GRID DATATABLE ROOT COMPONENT ---
  return (
    <div className="flex-1 bg-white p-8 min-h-screen flex flex-col">
      <h1 className="text-3xl font-bold text-[#17466c] tracking-tight mb-6">Housekeeping</h1>

      <div className="flex-1 bg-white border border-slate-100 rounded-xl p-6 shadow-sm flex flex-col">
        {/* Table Filter Operations Control Panel */}
        ...
        {/* Data Frame Grid Table Window element */}
                ...
                <tr key={ticket.id} className="hover:bg-slate-50/40 transition-colors">
                  ...
                  <td className="px-4 py-3.5">
                    {ticket.assignedStaff.isUnassigned ? (
                      <span className="text-rose-500 font-bold bg-rose-50 px-2 py-0.5 rounded border border-rose-100/50">Unassigned</span>
                    ) : (
                  ...
                  <td className="px-4 py-3.5">
                    <span className={`inline-flex items-center text-[10px] px-2.5 py-0.5 rounded-full font-bold text-white ${
                      ...
                      ticket.status === "For Review" ? "bg-[#1d5999]" : "bg-emerald-600"
                    }`}>
                      {ticket.status}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3.5 text-center">
                    <button
                      ...
                      className="bg-[#248ad6] text-white font-bold text-[11px] px-3 py-1 rounded shadow-sm"
                    >
                      View
                    </button>
                  </td>
      ...
      {/* Inject Modal Frame Controller Layer */}
